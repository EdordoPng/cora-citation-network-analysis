...
Indice 
  ...
  --- Percolation (da valutare perchè secondo me poco sensata)

  2 : Small-world effect (da valutare perchè secondo me poco sensata)
...
# Trova TUTTI i blocchi con coesione massima
max_coe <- max(coe)
...
# Plotta ogni blocco con coesione massima
par(mfrow = c(2, 4), mar = c(1, 1, 2, 1))  # 2 righe, 4 colonne per 7 grafici

for (i in seq_along(idx_max)) {
  h <- bg[[ idx_max[i] ]]
  
  # Assegna colori in base alla categoria
  V(h)$color <- cat_colors[ V(h)$category ]
  
  plot(
    h,
    vertex.size = 12,
    vertex.label = V(h)$name,     # usa name se vuoi gli ID originali
...
# Aggiungi una legenda in un pannello vuoto
plot.new()
legend(
  "center",
  legend = categories,
...
L'idea ora è di mostrare la rete con tutti i sottogruppi.


```{r fig.width=12, fig.height=9, fig.align='center', warning=FALSE}

# Trova tutti i blocchi con coesione = max - 1
...
n_plots <- length(idx_target) + 1  # +1 per la legenda
n_cols <- 4
n_rows <- ceiling(n_plots / n_cols)

# Plotta ogni blocco con coesione k = 3
...
# Aggiungi legenda in un pannello vuoto
plot.new()
legend(
  "center",
  legend = categories,
...
# Estrai il sottografo della SCC massima
...
```
Notare come in questo SCC ci siano dei paper che si citano a vicenda, il che risulta leggermente controintuitivo in una citation network.

## Percolation

Funzione per eseguire la Percolation.
...
Valutiamo qui 5 strategie di rimozione dei nodi.

1 Rimozione random 
2 Rimuovo prima quelli col grado più alto
3 Rimuovo prima quelli col PageRank più alto
4 Rimuovo prima quelli con la Betweenness più alta
5 Rimuovo prima quelli con la Closeness più alta
...
# random
rand = percolate(graph, size, d = sample(V(graph), size))    
# degree
indeg = percolate(graph, size, d = degree(graph, mode = "in"))    
# pagerank
pr = percolate(graph, size, d=page_rank(graph)$vector)    
# betweenness
bet = percolate(graph, size, d = betweenness(graph))    
# closeness
clo = percolate(graph, size, d = closeness(graph))    

```

Plotto quanto ottenuto.

```{r}

plot(0:size, indeg, type = "l", col=1, 
     xlab="Number of removed nodes", 
     ylab="Size of giant component")
lines(0:size, pr, col=2)
lines(0:size, bet, col=3)
lines(0:size, clo, col=4)
lines(0:size, rand, col=5)
lines(0:size, rep(vcount(graph)/2, size+1), lty=2)
legend(x = "bottomleft", 
       legend = c("in-deg", "pr", "btw", "clo", "rand"), lty = 1, col = 1:5)
