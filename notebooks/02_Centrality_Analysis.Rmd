...
Ho che 3 fattori distinti determinano il Pag Rank di un nodo : 
...
# Plot Authority con nodi colorati
plot(
  igraph_sub,
  layout = coords,
...
Un nodo è importante se se è linkdato da nodi importanti o se riceve tanti archi entranti.
...
In qusto codice vado infine a mostrare la rete ottenuta facendo la join tra i 100 nodi con katz più alta ed i 100 nodi con Power più alta.

```{r fig.width=12, fig.height=9, fig.align='center', warning=FALSE}


# Estraggo i nomi dei nodi dal subgrafo
sg_names <- subgraph_tbl %>% as_tibble() %>% pull(name)

# Faccio subset della tabella/df con i dati solo di quei nodi
...
# Porto tutto dentro subgraph_tbl per ggraph
...
# Trova le 30 coppie (i, j) con più paper citati in comune
top_cited <- which(common_cited == max(common_cited), arr.ind = TRUE)
top_cited_df <- as.data.frame(top_cited)
top_cited_df$shared_cited = common_cited[top_cited]

# Trova le 30 coppie con più paper citanti in comune
top_citing <- which(common_citing == max(common_citing), arr.ind = TRUE)
top_citing_df <- as.data.frame(top_citing)
top_citing_df$shared_citing = common_citing[top_citing]
...
get_top_pairs <- function(mat, topn = 30) {
  ...
  vals <- mat[ind]
  ord <- order(vals, decreasing = TRUE)[1:topn]
  data.frame(
    paper_i = ind[ord, "row"],
    paper_j = ind[ord, "col"],
    shared = vals[ord]
...
# (Facoltativo: aggiungi titolo/autore ai risultati usando le label dei nodi)
# V(graph)$label oppure il campo desiderato
