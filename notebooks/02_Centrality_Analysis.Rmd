...
un paper ha alta Katz se riceve citazioni da paper che a loro volta ricevono molte citazioni. 

Calcola il raggio spettrale r (massimo autovalore in valore assoluto)

Usa alpha = 0.85/r (damping factor) per garantire convergenza (serve alpha < 1/r)

La funzione alpha_centrality() implementa la formula di Katz
...
r = max(abs(eig))
alpha <- 0.85 / r
katz <- alpha_centrality(ig_graph, alpha = alpha)
...
Questo plot riassume e comunica efficacemente quali categorie sono più centrali, intermediarie o ben collegate nella rete secondo le diverse misure.

```{r fig.width=12, fig.height=9, fig.align='center', warning=FALSE}


# Supponiamo tu abbia la tabella compare come data.frame/tibble
...
compare_long <- compare %>%
  ...
  # Puoi rinominare le metriche per grafico più leggibile
...
# Grafico a barre con faccette per metrica
...
```{r}
# Usa solo le colonne delle metriche dalla tabella compare
M <- compare %>% 
...
Le metriche di grado e Katz formano un cluster di misure fortemente correlate che riflettono l’attività immediata e l’influenza cumulativa nei network. Pagerank e Closeness invece evidenziano aspetti più distinti della centralità, spesso inversamente correlati, rivelando diverse modalità di importanza nel network.
...
Implementa l'algoritmo iterativo di Bonacich Power Centrality
...
Alta Power = Paper che può raggiungere/influenzare molti altri paper attraverso la catena di citazioni
...
ggplot(nodes, aes(x = katz, y = power)) +
  geom_point(alpha = 0.3) +
  geom_label_repel(data = top_nodes, aes(label = name), max.overlaps = 15) +
  geom_hline(yintercept = q_power, linetype = "dashed", alpha = 0.5) +
  geom_vline(xintercept = q_katz, linetype = "dashed", alpha = 0.5) +
  labs(title = "Confronto Katz e Power centrality", x = "Katz Centrality", y = "Power Centrality")
...
In qusto codicevado a mostrare la rete ottenuta facendo la join tra i 100 nodi con katz più alta ed i 100 nodi con Powe più alta.

```{r fig.width=12, fig.height=9, fig.align='center', warning=FALSE}


# Estrai i veri nomi dei nodi dal subgrafo
sg_names <- subgraph_tbl %>% as_tibble() %>% pull(name)

# Fai subset della tabella/df con i dati solo di quei nodi
sg_metrics <- top_nodes %>%
  filter(name %in% sg_names) %>%
  mutate(
    katz_bin = cut(katz, breaks = quantile(katz, probs = seq(0, 1, by = 0.2), na.rm = TRUE), include.lowest = TRUE),
    power_bin = cut(power, breaks = quantile(power, probs = seq(0, 1, by = 0.2), na.rm = TRUE), include.lowest = TRUE)
  )

# Porta tutto dentro subgraph_tbl per ggraph
subgraph_tbl <- subgraph_tbl %>%
  left_join(sg_metrics, by = "name")

# Grafico con Katz grandezza dei nodi 
set.seed(99)
ggraph(subgraph_tbl, layout = "lgl") +
  geom_edge_link(alpha = 0.4) +
  geom_node_point(aes(size = katz_bin, color = category)) +
  geom_node_text(aes(label = name), repel=TRUE, size = 2.7) +
  scale_color_brewer(palette = "Set2") +
  theme_graph()


# Grafico con Power grandezza dei nodi
set.seed(99)
ggraph(subgraph_tbl, layout = "lgl") +
  geom_edge_link(alpha = 0.4) +
  geom_node_point(aes(size = power_bin, color = category)) +
  geom_node_text(aes(label = name), repel = TRUE, size = 2.7) +
  scale_color_brewer(palette = "Set2") +
  theme_graph()
```
