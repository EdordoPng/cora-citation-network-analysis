...
# Indice
  ...
  3 : Analisi di Eterogeneità
    ...
    3.1: Shannon Entropy
        ...
        Calcolo Shannon Entropy per ogni nodo (basato su predecessori, in-neighbors)
        
        Creo il dataframe per fare l'analisi
    ...
    3.2: Simpson Diversity
       ...
       Formula e funzione della Simpson Diversity
       
       Calcolo della distribuzione delle categorie dei predecessori

       Creo il dataframe per l'analisi e mostro le statistiche
    ...
    3.3: Rao Quadratic Entropy
       ...
       Step 1 : Creo la matrice indicatrice C [n x k]

                Creo la matrice di flusso F [n x n] (quante citazioni esistono tra categorie di paper.)
       ...
       Step 8 : Tabella : Top paper interdisciplinari
      
       Step 9 : Tabella : Top paper specializzati (Rao basso, ma con citazioni)
  ...
  5 : Confronto tra Similarità e Centralità
    ...
    Domande di ricerca :
      ...
      A. I paper con alta centralità (es. PageRank, betweenness) tendono ad essere interdisciplinari ?
          ...
          - Creazione dataframe completo, filtro i paper con out-degree = 0
   
          - Threshold per "alta centralità" (top 10%) 
...
# Step 0: Definisco nodes dal grafo di partenza
nodes <- data.frame(
  name = V(graph)$name,
  category = V(graph)$category
)

# Step 1: top 500 coppie per cosine similarity (solo 0 < sim < 1)
...
# Step 2: Creo il grafo pesato
sim_graph <- graph_from_data_frame(similarity_df, directed = FALSE)


# Porto gli attributi categoria dal grafo originale, se presenti
...
Distribuzione della Pearson similarity
...
### 7. Network delle 200 coppie di nodi con più alto valore di Global similarity.
...
ggraph(graph_global, layout = "fr") +
  ...
  theme_graph() +
  labs(
    title = "Network delle migliori 200 coppie per Global Similarity",
    color = "Categoria",
...
Creo una distribuzione di probabilità sui vicini.
...
```
La Media : 0.2029 ,indica che in media appunto i paper ricevono citazioni da un insieme abbastanza omogeneo di categorie
...
Per ogni nodo, calcolo della distribuzione delle categorie dei predecessori

```{r}

simpson_values <- sapply(V(graph), function(node) {
  # Ottieni predecessori 
  preds <- neighbors(graph, node, mode = "in")
...
Esempio: Una disciplina è molto eterogenea (interdisciplinare) se cita molte discipline diverse tra loro in proporzioni simili.
...
## 4 : Analisi a livello Categoria

### 4.1 Discipline più e meno autocitanti 

Voglio cercare di capir quali sono le discipline che si autocitano di più e quali invece lo fanno meno

```{r fig.width=12, fig.height=9, fig.align='center', warning=FALSE}

edges <- readRDS("../data/processed/edges.rds")

edges_cat_df <- edges %>%
  mutate(
    category_from = V(graph)$category[match(from, V(graph)$name)],
    category_to   = V(graph)$category[match(to, V(graph)$name)]
  )

# edges_cat_df è già la tabella archi con categoria sorgente/destinazione

# 1. Conta i flussi tra categorie
flows_cat <- edges_cat_df %>%
  count(category_from, category_to, name = "n_citations")

# 2. Calcola la size (numero di paper) per ciascuna categoria
size_cat <- as.data.frame(table(V(graph)$category))
colnames(size_cat) <- c("category", "size")

# 3. Calcola autocitazione, citazioni ricevute, citazioni inviate
flows_summary <- size_cat %>%
  left_join(
    flows_cat %>% 
      filter(category_from == category_to) %>% 
      group_by(category_from) %>% 
      summarise(self_citation = sum(n_citations)), 
    by = c("category" = "category_from")
  ) %>%
  left_join(
    flows_cat %>% 
      filter(category_from != category_to) %>%
      group_by(category_to) %>% 
      summarise(in_citation = sum(n_citations)), 
    by = c("category" = "category_to")
  ) %>%
  left_join(
    flows_cat %>% 
      filter(category_from != category_to) %>%
      group_by(category_from) %>% 
      summarise(out_citation = sum(n_citations)), 
    by = c("category" = "category_from")
  )

# 4. Calcola le metriche normalizzate 
flows_summary <- flows_summary %>%
  mutate(self_per_paper = self_citation / size,
         in_per_paper   = in_citation / size,
         out_per_paper  = out_citation / size)

# 5. Ordina e mostra le discipline più autocitanti e più citate
flows_summary %>% arrange(desc(self_per_paper))
flows_summary %>% arrange(desc(in_per_paper))


```

Avevamo già calcolato in precedenza la matrice di flusso F.
...
### 4.2 Creo la heatmap della matrice X-test tra categorie

rappresenta quanto il flusso di citazioni da una categoria (sull’asse Y, “sorgente”) verso un’altra categoria (sull’asse X, “destinazione”) è superiore o inferiore a quanto atteso da un modello casuale che tiene conto solo dell’attività di citazione e di essere citati di ciascuna categoria.



```{r fig.width=12, fig.height=9, fig.align='center', warning=FALSE}

# Supponi di avere già la matrice X_test 7x7 e il vettore categories
# Assicurati che X_test abbia i nomi riga/colonna giusti!
rownames(X_test) <- categories
colnames(X_test) <- categories

# Rimuovi le autocitazioni (diagonale)
diag(X_test) <- 0

# Porta la matrice in formato 'long' per ggplot
df <- melt(X_test)
colnames(df) <- c("i", "j", "value")

# Calcola il massimo assoluto per scalare la palette colore
m <- max(abs(df$value), na.rm = TRUE)
...
ggplot(df, aes(j, i, fill = value)) +
  ...
  labs(
    title = "X-test fra categorie\nFlusso osservato vs atteso",
    x = "Categoria j (Destinazione)",
    y = "Categoria i (Sorgente)",
    fill = "X-test"
  ) +
  theme(
...
Le celle in blu indicano che ci sono meno citazioni del previsto, cioè flusso o scarso collegamento rispetto a quanto ci si aspetterebbe casualmente.

```{r fig.width=12, fig.height=9, fig.align='center', warning=FALSE}

# Porta X_test in formato long
df_edges <- reshape2::melt(X_test)
colnames(df_edges) <- c("from", "to", "weight")

# (Opzionale) rimuovi diagonale e tieni solo archi con peso abbastanza grande
threshold <- 1  # scegli tu la soglia
df_edges <- df_edges %>%
  filter(from != to, !is.na(weight), abs(weight) > threshold)

g <- igraph::graph_from_data_frame(
  d = df_edges,
  vertices = data.frame(name = categories),
  directed = TRUE
)
...
ggraph(g, layout = "circle") +
  ...
  geom_node_text(aes(label = name), size = 5, vjust = 2.5) +
  coord_fixed() +
  theme_graph() +
  labs(
    title = "Flusso anomalo osservato vs atteso tra categorie",
    subtitle = "Spessore e colore indicano l'entità dell'anomalia (X-test)"
  )

```

### 4.3 Troviamo le top 5 categorie per interdisciplinarietà

Prima definiamo le funzioni che servono nel calcolo
...
heterogeneity <- function(g, D, mode = "col") {
  ...
  return(list(
    shannon = apply(A, dim, shannon), 
    simpson = apply(A, dim, simpson), 
    rao = apply(A, dim, rao, D)
  ))
}

# Flusso tra categorie

# 1. Grafo di flusso
f <- graph_from_adjacency_matrix(F, mode = "directed", weighted = TRUE)
V(f)$name <- categories

# 2. Similarità e matrice distanza
S <- similarity(f)         # Similarità tipo cosine (default)
D <- 1 - S                 # Distanza per Rao

# 3. Eterogeneità (Rao)
het <- heterogeneity(f, D)
rao_scores <- het$rao

# Dataframe categorie e Rao 
flows_summary$rao_entropy <- rao_scores 

# Top 5 categorie più interdisciplinari
flows_summary %>%
  select(category, rao_entropy) %>%
  arrange(desc(rao_entropy)) %>%
  head(5)
...
### Domanda A : I paper con alta centralità (es. PageRank, Betweenness) tendono ad essere interdisciplinari?

Procedimento : 

  - Creo il dataframe completo e filtro i paper con out-degree = 0
   
  - Setto una threshold per "alta centralità" (top 10%) 
...
# Identificazione paper con alta Centralità

# Definiamo una threshold per "alta centralità" (top 10%)
...
### Domanda B : I paper interdisciplinari (alto Rao da out-degree) sono più influenti (alto in-degree/PageRank) ?
...
# Assumo che df_active esista già e contenga:
# rao_entropy, betweenness, pagerank, category
...
# Crea la colonna 'tipo' se non esiste
...
### Domanda C : Quali categorie producono i paper più interdisciplinari? E quali beneficiano di più dai ponti interdisciplinari?
...
# 1 Interdisciplinarità per Categoria
...
# 2 % Paper Interdisciplinari per Categoria
...
# 3 Top Coppie di categorie per numero di citazioni (paper 1 cita paper 2 tot volte)

cat("3 - Pattern di Citazioni CROSS-Categoria\n")
cat("----------------------------------------------------------------------------\n\n")
...
# 4: Chi è il maggior ESPORTATORE vs IMPORTATORE?

cat("4 - Ruolo delle Categorie nel Network\n")
cat("----------------------------------------------------------------------------\n\n")
...
print(kable(trade_balance,
            col.names = c("Categoria", "Esporta (cita out)", "Importa (citata da)", "Balance"),
            digits = 0))
...
# ============================================================================
# SUMMARY FINALE E SALVATAGGIO DATI
# ============================================================================

cat("\n\n")
cat("============================================================================\n")
cat("RIEPILOGO RISULTATI CHIAVE\n")
cat("============================================================================\n\n")

cat("📊 DATASET:\n")
cat("  - Paper totali:", vcount(graph), "\n")
cat("  - Categorie:", length(unique(V(graph)$category)), "\n")
cat("  - Citazioni:", ecount(graph), "\n\n")

cat("🔍 INTERDISCIPLINARITÀ (Rao Entropy):\n")
inter_count <- sum(V(graph)$rao_entropy > 0)
spec_count <- sum(V(graph)$rao_entropy == 0 & V(graph)$outdegree > 0)
term_count <- sum(V(graph)$outdegree == 0)

cat("  - Paper interdisciplinari (Rao > 0):", inter_count, 
    paste0("(", round(inter_count / vcount(graph) * 100, 1), "%)"), "\n")
cat("  - Paper specializzati (Rao = 0, con citazioni):", spec_count,
    paste0("(", round(spec_count / vcount(graph) * 100, 1), "%)"), "\n")
cat("  - Paper terminali (nessuna citazione out):", term_count,
    paste0("(", round(term_count / vcount(graph) * 100, 1), "%)"), "\n")
cat("  - Rao medio:", round(mean(V(graph)$rao_entropy), 4), "\n")
cat("  - Rao massimo:", round(max(V(graph)$rao_entropy), 4), "\n\n")

cat("🌉 RUOLO DEGLI INTERDISCIPLINARI:\n")
inter_papers <- V(graph)$rao_entropy > 0
spec_papers <- V(graph)$rao_entropy == 0 & V(graph)$outdegree > 0

betw_inter <- mean(V(graph)$betweenness[inter_papers])
betw_spec <- mean(V(graph)$betweenness[spec_papers])

cat("  - Betweenness media (interdisciplinari):", round(betw_inter, 1), "\n")
cat("  - Betweenness media (specializzati):", round(betw_spec, 1), "\n")
cat("  - Rapporto:", round(betw_inter / betw_spec, 1), "×\n")
cat("  → Gli interdisciplinari sono PONTI critici nella rete!\n\n")

cat("📚 CATEGORIE:\n")
cat("  - Più interdisciplinare:", category_stats$category[1], 
    paste0("(", round(category_stats$perc_interdisciplinari[1], 1), "% paper interdisciplinari)"), "\n")
cat("  - Più specializzata:", category_stats$category[nrow(category_stats)],
    paste0("(", round(category_stats$perc_interdisciplinari[nrow(category_stats)], 1), "% paper interdisciplinari)"), "\n")
cat("  - Flusso cross-categoria principale:", 
    cross_summary$from[1], "→", cross_summary$to[1], 
    paste0("(", cross_summary$total_citations[1], " citazioni)"), "\n\n")

cat("💡 TAKE-HOME MESSAGES:\n")
cat("  1. Solo", round(inter_count / vcount(graph) * 100, 1), 
    "% dei paper sono interdisciplinari, ma hanno ruolo strategico\n")
cat("  2. Interdisciplinari = ponti (betweenness", round(betw_inter / betw_spec, 1),
    "× più alta), NON superstar\n")
cat("  3. Categorie hanno livelli diversi di apertura disciplinare\n")
cat("  4. Flussi cross-disciplinari rivelano gerarchie epistemiche\n\n")
...
cat("============================================================================\n")
cat("SALVATAGGIO DATI PER ANALISI FUTURE\n")
cat("============================================================================\n\n")
...
if (!dir.exists(results_dir)) {
  ...
  cat("Directory creata:", results_dir, "\n\n")
}

# 1. GRAFO COMPLETO con attributo Rao
cat("1. Salvataggio grafo con Rao Entropy...\n")
save(graph, file = file.path(results_dir, "graph_with_rao.RData"))
cat("   ✓ File:", file.path(results_dir, "graph_with_rao.RData"), "\n\n")

# 2. DATAFRAME COMPLETO per analisi statistiche
cat("2. Salvataggio dataframe con tutte le metriche...\n")
df_complete <- data.frame(
  paper = V(graph)$name,
  category = V(graph)$category,
  rao_entropy = V(graph)$rao_entropy,
  pagerank = V(graph)$pagerank,
  betweenness = V(graph)$betweenness,
  degree_in = V(graph)$indegree,
  degree_out = V(graph)$outdegree,
  katz = V(graph)$katz,
  stringsAsFactors = FALSE
)
write.csv(df_complete, file.path(results_dir, "paper_metrics.csv"), row.names = FALSE)
cat("   ✓ File:", file.path(results_dir, "paper_metrics.csv"), "\n")
cat("   ✓ Righe:", nrow(df_complete), "| Colonne:", ncol(df_complete), "\n\n")

# 3. STATISTICHE PER CATEGORIA
cat("3. Salvataggio statistiche aggregate per categoria...\n")
write.csv(category_stats, file.path(results_dir, "category_stats.csv"), row.names = FALSE)
cat("   ✓ File:", file.path(results_dir, "category_stats.csv"), "\n")
cat("   ✓ Categorie:", nrow(category_stats), "\n\n")

# 4. FLUSSI CROSS-CATEGORIA
cat("4. Salvataggio flussi citazioni cross-categoria...\n")
write.csv(cross_summary, file.path(results_dir, "cross_category_flows.csv"), row.names = FALSE)
cat("   ✓ File:", file.path(results_dir, "cross_category_flows.csv"), "\n")
cat("   ✓ Coppie (from→to):", nrow(cross_summary), "\n")
cat("   ✓ Totale citazioni cross:", sum(cross_summary$total_citations), "\n\n")

cat("============================================================================\n")
cat("✓ Tutti i file salvati con successo in:", results_dir, "\n")
cat("============================================================================\n\n")

cat("File pronti per:\n")
cat("  • Analisi future in R: caricare 'graph_with_rao.RData'\n")
cat("  • Analisi statistica esterna: aprire 'paper_metrics.csv'\n")
cat("  • Report/presentazioni: usare 'category_stats.csv'\n")
cat("  • Network visualization: usare 'cross_category_flows.csv'\n\n")
